...
const setCALHash = (currency: CryptoCurrency, hash: string): string => {
  CALHashByChainIdMap.set(currency, hash);
  return CALHashByChainIdMap.get(currency)!;
};

const getCALHash = (currency: CryptoCurrency): string => {
  if (!CALHashByChainIdMap.has(currency)) {
    CALHashByChainIdMap.set(
      currency,
      embeddedHashesByChainId[
        (currency?.ethereumLikeInfo?.chainId || "") as keyof typeof embeddedHashesByChainId
      ],
    );
  }
